...
function setupIngestionUI() {
    ...
    // Select Multi-Files
    const btnSelectFiles = document.getElementById('btnSelectBatchFiles');
    const hiddenFileInput = document.getElementById('batchHiddenFileInput');
    if (btnSelectFiles) {
    ...
    if (hiddenFileInput) {
        hiddenFileInput.addEventListener('change', (e) => {
            ...
            const validPaths = files.map(f => f.path).filter(Boolean);
            if (validPaths.length > 0) {
                selectedLocalFiles = validPaths;
                const summary = document.getElementById('batchLocalFilesSummary');
                if (summary) {
                    summary.style.display = 'block';
                    summary.textContent = `🎬 Đã chọn ${validPaths.length} file video`;
                }
                window.showToast?.(`🎬 Đã chọn ${validPaths.length} file video!`, 'success');
            } else if (files.length > 0) {
                window.showToast?.('⚠️ Trình duyệt web không cấp đường dẫn tệp tuyệt đối. Vui lòng bấm "Chọn Folder" để nạp video!', 'warning');
            }
        });
    }
    ...
    // Preset Selector Change Listener (Dynamic Panels)
    const selectPreset = document.getElementById('batchSelectPreset');
    const panelReview = document.getElementById('batchPanelPresetReview');
    const panelDubbing = document.getElementById('batchPanelPresetDubbing');
    const panelAnti = document.getElementById('batchPanelPresetAnti');

    if (selectPreset) {
        selectPreset.addEventListener('change', () => {
            const p = selectPreset.value;
            if (panelReview) panelReview.style.display = (p === 'review') ? 'flex' : 'none';
            if (panelDubbing) panelDubbing.style.display = (p === 'dubbing') ? 'flex' : 'none';
            if (panelAnti) panelAnti.style.display = (p === 'anti_copyright') ? 'flex' : 'none';
        });
    }

    // Target Duration Change Listener
    const targetMinutesSelect = document.getElementById('batchTargetMinutes');
    const wordsHint = document.getElementById('batchWordsEstimateHint');
    if (targetMinutesSelect && wordsHint) {
        targetMinutesSelect.addEventListener('change', () => {
            ...
            wordsHint.textContent = `~${words.toLocaleString()} từ (3.5 từ/s)`;
        });
    }
...
async function fetchPreflightStatus() {
    try {
        const res = await fetch('/api/batch/preflight');
        if (!res.ok) return;
        const data = await res.json();
        
        const elFfmpeg = document.getElementById('batchStatusFfmpeg');
        if (elFfmpeg) {
            if (data.ffmpeg && data.ffmpeg.ready) {
                elFfmpeg.innerHTML = '<span style="font-size: 8px;">●</span> FFmpeg: Sẵn sàng';
                elFfmpeg.style.color = '#10b981';
            } else {
                elFfmpeg.innerHTML = '<span style="font-size: 8px;">●</span> FFmpeg: ⚠️ Chưa có';
                elFfmpeg.style.color = '#f87171';
            }
        }

        const elAsr = document.getElementById('batchStatusAsr');
        if (elAsr) {
            if (data.asr && data.asr.ready) {
                elAsr.innerHTML = '<span style="font-size: 8px;">●</span> Whisper ASR: Sẵn sàng';
                elAsr.style.color = '#10b981';
            } else {
                elAsr.innerHTML = '<span style="font-size: 8px;">●</span> Whisper ASR: ⚠️ Cần nạp';
                elAsr.style.color = '#fbbf24';
            }
        }

        const elAi = document.getElementById('batchStatusAi');
        if (elAi) {
            if (data.ai_key && data.ai_key.ready) {
                elAi.innerHTML = '<span style="font-size: 8px;">●</span> AI Key: Đã nạp';
                elAi.style.color = '#a855f7';
            } else {
                elAi.innerHTML = '<span style="font-size: 8px;">●</span> AI Key: ⚠️ Cần cấu hình';
                elAi.style.color = '#fbbf24';
            }
        }
    } catch (e) {
        console.warn("Không thể kiểm tra Preflight:", e);
    }
}
...
async function handleAddTasksToQueue() {
    ...
    if (preset === 'review') {
        preset_config = {
            ...preset_config,
            target_minutes: parseFloat(document.getElementById('batchTargetMinutes')?.value || 5.0),
            review_style: document.getElementById('batchSelectReviewStyle')?.value || 'dramatic',
            voice_id: document.getElementById('batchSelectVoice')?.value || 'ngoc_huyen',
            voice_speed: parseFloat(document.getElementById('batchVoiceSpeed')?.value || 1.0),
            aspect_ratio: document.getElementById('batchSelectAspect')?.value || '9:16',
            bgm_volume: parseInt(document.getElementById('batchBgmVolume')?.value || 12),
            auto_subtitles: document.getElementById('batchChkAutoSubtitles')?.checked !== false
        };
    ...
    } else if (preset === 'anti_copyright') {
        preset_config = {
            ...preset_config,
            speed: parseFloat(document.getElementById('batchAntiSpeed')?.value || 1.05),
            zoom: parseFloat(document.getElementById('batchAntiZoom')?.value || 1.05),
            aspect_ratio: document.getElementById('batchAntiAspect')?.value || '9:16',
            mirror: document.getElementById('batchChkMirror')?.checked !== false
        };
    }
